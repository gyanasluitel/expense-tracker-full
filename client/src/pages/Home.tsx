...
const Dashboard = () => {
    ...
    const { recentTransactions, loadingRecentTransactions } = useAppSelector((state) => state.transactions);
    const { totalIncome, totalExpenses } = recentTransactions.stats;
    const netBalance = totalIncome - totalExpenses;

    useEffect(() => {
        if (userId) dispatch(fetchRecentTransactions({
            userId,
            page: 1,
            limit: 5
        }));
